...
server <- function(input, output, session) {
  ...
  output$cox_results <- renderPrint({
    ...
    # Ensure factor levels are set properly
    dat$ARM <- relevel(factor(dat$ARM), ref = "Control")
    dat$SEX <- relevel(factor(dat$SEX), ref = "M")
    dat$BIOMARKER <- relevel(factor(dat$BIOMARKER), ref = "Negative")
  ...
  output$forest_plot <- renderPlot({
    ...
    tryCatch({
      ...
      if (nrow(coef_summary$coefficients) == 0) {
        plot.new()
        text(0.5, 0.5, "No coefficients to display", cex = 1.5)
        return()
      }
      ...
      coef_data$Variable_Clean <- coef_data$Variable
      coef_data$Variable_Clean <- gsub("ARMExperimental", "Treatment (Experimental vs Control)", coef_data$Variable_Clean)
      coef_data$Variable_Clean <- gsub("AGE", "Age (per year)", coef_data$Variable_Clean)
      coef_data$Variable_Clean <- gsub("SEXF", "Sex (Female vs Male)", coef_data$Variable_Clean)
      coef_data$Variable_Clean <- gsub("ECOG", "ECOG Score ", coef_data$Variable_Clean)
      coef_data$Variable_Clean <- gsub("STAGEII", "Stage II vs I", coef_data$Variable_Clean)
      coef_data$Variable_Clean <- gsub("STAGEIII", "Stage III vs I", coef_data$Variable_Clean)
      coef_data$Variable_Clean <- gsub("STAGEIV", "Stage IV vs I", coef_data$Variable_Clean)
      coef_data$Variable_Clean <- gsub("BIOMARKERPositive", "Biomarker (Positive vs Negative)", coef_data$Variable_Clean)
      coef_data$Variable_Clean <- gsub("BIOMARKERUnknown", "Biomarker (Unknown vs Negative)", coef_data$Variable_Clean)
      ...
      coef_data$p_formatted <- ifelse(coef_data$p_value < 0.001, "<0.001", sprintf("%.3f", coef_data$p_value))
      
      # Create table text for forest plot - FIXED FORMAT
      # Add header row
      tabletext <- cbind(
        c("Variable", coef_data$Variable_Clean),
        c("HR", coef_data$HR_formatted),
        c("95% CI", coef_data$CI_formatted),
        c("P-value", coef_data$p_formatted)
      )
      ...
      # Create forest plot with proper data structure
      forestplot(
        tabletext,
        mean = c(NA, coef_data$HR),
        lower = c(NA, coef_data$Lower),
        upper = c(NA, coef_data$Upper),
        zero = 1,
        cex = 0.9,
        boxsize = 0.3,
        col = theme_colors,
        ...
        xlim = c(0.1, 5),
        xticks = c(0.1, 0.2, 0.5, 1, 2, 5),
        grid = TRUE,
        is.summary = c(TRUE, rep(FALSE, nrow(coef_data))),
      ...
      plot.new()
      text(0.5, 0.5, paste("Error in Cox model:", e$message), cex = 1.2)
    })
  })
  ...
  output$forest_table <- renderDT({
    ...
    tryCatch({
      ...
      if (nrow(coef_summary$coefficients) == 0) {
        return(datatable(data.frame(Message = "No coefficients to display"), 
      ...
      coef_data$Variable <- gsub("ECOG", "ECOG Score ", coef_data$Variable)
      coef_data$Variable <- gsub("STAGEII", "Stage II vs I", coef_data$Variable)
      coef_data$Variable <- gsub("STAGEIII", "Stage III vs I", coef_data$Variable)
      coef_data$Variable <- gsub("STAGEIV", "Stage IV vs I", coef_data$Variable)
      coef_data$Variable <- gsub("BIOMARKERPositive", "Biomarker (Positive vs Negative)", coef_data$Variable)
      coef_data$Variable <- gsub("BIOMARKERUnknown", "Biomarker (Unknown vs Negative)", coef_data$Variable)
      
      # Format numbers
      coef_data$HR_formatted <- sprintf("%.3f", coef_data$HR)
      coef_data$CI_formatted <- sprintf("%.3f - %.3f", coef_data$Lower_CI, coef_data$Upper_CI)
      coef_data$p_formatted <- ifelse(coef_data$p_value < 0.001, "<0.001", sprintf("%.4f", coef_data$p_value))
      ...
      datatable(data.frame(Error = paste("Error in Cox model:", e$message)), 
                options = list(dom = 't'), rownames = FALSE)
    })
  })
}
